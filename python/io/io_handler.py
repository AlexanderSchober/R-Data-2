...
from ..io import file_methods as file
import os

class IO_Raw_Handler:
    '''
    ##############################################
    ...
    def scan_directory(self):
        ...
        ##############################################
        '''
        self.raw_file_list = file.get_files_in_folder(
            self.directory_path, 
            extension = '.txt')
    ...
    def evaluate_files(self):
        ...
        try:
            self.common_str = file.get_common_substrings(self.file_list)
        except: 
            pass
    ...
    def process_import(self):
        ...
        ##############################################
        #grab the dimensions the user wants to keep
        dimension_list = []
...
class IO_Raw_Worker:
    ...
    def run(self):
        ...
        for file in self.file_list:
            try:
                future_axes_pos.append(
                    [ element[6].index(
                        float(file.split(element[2])[1].split(element[3])[0]) )
                    for element in self.dimension_list]
                )
                ...
                future_axes_pos.append(
                    [ element[6].index(
                        file.split(element[2])[1].split(element[3])[0]) 
                    for element in self.dimension_list]
                )
        ...
        for file in self.file_list:
            future_data_array.append([*self._reader(file)])

        ##############################################
        #send it all out to the writer
        axis_string = self._axis_writer(
            future_axes_values,
            future_axes_names,
            future_axes_units
        )

        data_string = self._data_writer(
            future_axes_pos,
            future_data_array
        )
    ...
    def _axis_writer(self, future_axes_values, future_axes_names, future_axes_units):
        '''
        ##############################################
        This method will generate a string with the 
        input being a list of strings that are
        ...
        for i in range(len(future_axes_names)):
            ...
            output += self._list_to_string(temp_list) + "\n"

        return output

    def _data_writer(self,future_axes_pos,future_data_array):
        '''
        ##############################################
        This method will generate a string with the 
        input being a list of strings that are
        ...
        output += self._list_to_string(future_axes_pos) + "\n"
    ...
    def _list_to_string(self,list_item):
        '''
        ##############################################
