...
class IntLor:
    ...
    def Function(self,Parameters):
        '''
        ###############################################################################
        This will be the main function name and shoud thereofre 
        be called Function
        
        The first parameter will be x and should therefore be
        loaded. We recommend unfolding the actual elements and 
        giving them proper names to allows for a more
        ...
        y           = [quad(self.LorrPart,
                            0,
                            200e-6,
                            args = (x[i],Parameters))[0]
    ...
    def LorrPart(self,Var,x,Parameters):
        '''
        ##########################################################
        This is a bit special addition to the normal treatement
        This function is the actual gaussian part that will be 
        inegrated.
        ##########################################################
        '''
        #unfold x
        ...
        #process the function
        #process the function
        HWHM        = ((2*HWHM_0)/(1+numpy.exp(Assymetry*(Var - x))) )
        
        y           = (Amplitude*(HWHM**2)/(((Var - x-Position)**2)+HWHM**2))
